...
> - Build with **your own Personal Team** in Xcode (Signing & Capabilities) — ad-hoc signing silently blocks the camera/mic permission dialogs on macOS 14+.
> - **Grant Camera + Microphone** (and Accessibility, to type into the terminal) when prompted on first launch. *Eye tracking does nothing without Camera.*
...
- **Xcode** (the full IDE, not just the Command Line Tools) — required to build a macOS app. Install from the App Store; confirm with `xcodebuild -version`.
- **[XcodeGen](https://github.com/yonaskolb/XcodeGen)** — `brew install xcodegen`. The `.xcodeproj` is generated from `project.yml`; running `xcodegen generate` also creates the build scheme.
- An Apple ID / **Personal Team** in Xcode for signing (recommended — see Build; ad-hoc signing can silently block the camera/mic permission dialogs on macOS 14+).
...
cd eyeTerm
xcodegen generate   # regenerates eyeTerm.xcodeproj AND its shared build scheme
xcodebuild -scheme eyeTerm -configuration Debug -derivedDataPath ./build-debug build CODE_SIGNING_ALLOWED=NO
open build-debug/Build/Products/Debug/eyeTerm.app
```

> Run `xcodegen generate` first. The committed `.xcodeproj` may not include a *shared* scheme, in which case a headless `xcodebuild -scheme eyeTerm` fails with "scheme not found"; regenerating creates it. (If you skip XcodeGen, opening the project once in Xcode also creates the scheme.)

Or open `eyeTerm.xcodeproj` in Xcode and run directly. **Recommended:** use your Personal Team signing identity in Xcode — ad-hoc signing can silently block the camera/mic permission dialogs on macOS 14+.

### Voice models

WhisperKit auto-downloads its CoreML model from HuggingFace on first launch (~150 MB for `small.en`). No manual setup needed.

If you want to use the whisper.cpp backend instead, drop a ggml model into `Resources/Models/ggml/` (e.g. `ggml-small.en.bin` from [ggerganov/whisper.cpp](https://huggingface.co/ggerganov/whisper.cpp/tree/main)) before building.
...
**Settings window hides behind the overlay** — Should be fixed. If it happens, click the menu bar icon to bring focus back, then re-open Settings.

**Transcription shows garbled characters** — Fixed: the display now only renders complete words (was a WhisperKit partial-token artifact).
